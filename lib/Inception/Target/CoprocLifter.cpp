...
#include "inception/Target/ARM/ARMISD.h"
...
void CoprocLifter::MSRHandler(SDNode *N, IRBuilder<> *IRB) {
  ...
  if (DestNode == NULL) {
    klee::klee_error("[CoprocLifter] MSR Handler expected ConstantSDNode ...");
  }
  ...
  */
  ...
  switch (reg_id) {
    ...
    default:
      klee::klee_error("[MSRHandler] unsupported coproc register");
  }
...
void CoprocLifter::MRSHandler(SDNode *N, IRBuilder<> *IRB) {
  ...
  if (SrcNode == NULL) {
    klee::klee_error("[CoprocLifter] MRS Handler expected ConstantSDNode ...");
  }
  ...
  */
  switch(reg_id) {
    ...
    case 20: //CONTROL
      ...
      klee::klee_message("[MRSHandler] CONTROL only bit 1 is supported");
      break;
    default :
      klee::klee_error("[MRSHandler] unsupported coproc register");
  }
