...
#include "inception/Target/ARM/ARMBaseInfo.h"
#include "inception/Target/ARM/ARMLifterManager.h"
#include "inception/Target/ARM/DummyLifter.h"
...
void Decompiler::decompileBasicBlocks(MachineFunction *MF, Function *F,
  ...
  while (BI != BE) {
    if (decompileBasicBlock(BI, F, Address, entryAddress) == NULL) {
      klee::klee_error("Unable to decompile basic block!");
    }
  ...
  while (decompile) {
    ...
    for (Function::iterator I = ++F->begin(), E = F->end(); I != E; ++I) {
      if (I->empty()) {
        ...
        klee::klee_message("new address to decompile found %s: 0x%08x\n",
...
void Decompiler::splitBasicBlockIntoBlock(Function::iterator Src,
                                          ...
                                          BasicBlock *Tgt) {
  // klee::klee_message("splitBasicBlockIntoBlock:");
  ...
  // klee::klee_message("split before");
  // Src->dump();
  // Tgt->dump();
...
BasicBlock *Decompiler::decompileBasicBlock(MachineBasicBlock *MBB, Function *F,
  ...
  for (auto b = DAG->allnodes_begin(), e = DAG->allnodes_end(); b != e; b++) {
    ...
    if (solver != NULL) {
      ...
      // klee::klee_message("Entering lifter %s ", solver->name.c_str());
      ...
      if (it_start == true) {  //(it_state & 0b1111) != 0) {
        ...
        int cond = it_state >> 4;
        // klee::klee_message("it_state %08x cond %08x\n", it_state, cond);
        Value *Cmp = createCondition(cond, condIRB);
        if (Cmp != NULL) {
          condIRB->CreateCondBr(Cmp, codeBB, nextBB);
      ...
      klee::klee_error("Unable to find lifter for opcode %d", TargetOpc);
    }
  }
