...
#include "passes/BreakIntToPtr/BreakIntToPtr.hpp"
#include "passes/CleanFunctions/CleanFunctions.hpp"
...
public:
  Terrace() {
    mainModule = NULL;
    record_time = false, divergence = 1;
    main_fct = NULL;
    ...
    // sys::SetInterruptFunction(&Terrace::interrupt_handle);
  ...
  // // We init the memory model of Klee
  // void init_mem_model(TMRNormalReads *tmr_normal,
  //                     TMRSpecialReads *tmr_special){};
  ...
  void flat_control_flow();

  // We ask Klee to solved all constraints
  void generate_test_cases(){};

  // We backup our constraints in files
  void export_test_cases(){};

  void insert_main_and_icp(){};
