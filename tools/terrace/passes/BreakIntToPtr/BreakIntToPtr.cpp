...
/*
    ...
    along with Inception translator.  If not, see
...
//===- BreakIntToPtr.cpp - Change constant GEPs into GEP instructions -
//--//
//
...
//
struct BreakIntToPtr : public FunctionPass {
  static char ID;

  BreakIntToPtr() : FunctionPass(ID) {}
...
char BreakIntToPtr::ID = 42;

// Register the pass
static RegisterPass<BreakIntToPtr> P("break-constIntToPtr",
                                     "Remove IntToPtr Constant Expressions");

FunctionPass *createBreakIntToPtrPass() { return new BreakIntToPtr(); }
...
//
static ConstantExpr *hasConstantIntToPtr(Value *V) {
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(V)) {
    if (CE->getOpcode() == Instruction::IntToPtr) {
      ...
      for (unsigned index = 0; index < CE->getNumOperands(); ++index) {
        if (hasConstantIntToPtr(CE->getOperand(index)))
          return CE;
      }
    }
  }
...
static Instruction *convertExpression(ConstantExpr *CE, Instruction *InsertPt) {
  ...
  case Instruction::ICmp: {
    ...
    NewInst = CmpInst::Create(Op, llvm::CmpInst::Predicate(CE->getPredicate()),
...
//
bool BreakIntToPtr::runOnFunction(Function &F) {
  bool modified = false;
  ...
  for (Function::iterator BB = F.begin(); BB != F.end(); ++BB) {
    for (BasicBlock::iterator i = BB->begin(); i != BB->end(); ++i) {
      ...
      for (unsigned index = 0; index < I->getNumOperands(); ++index) {
        if (hasConstantIntToPtr(I->getOperand(index))) {
  ...
  while (Worklist.size()) {
    ...
    if (PHINode *PHI = dyn_cast<PHINode>(I)) {
      for (unsigned index = 0; index < PHI->getNumIncomingValues(); ++index) {
        ...
        if (ConstantExpr *CE =
                hasConstantIntToPtr(PHI->getIncomingValue(index))) {
      ...
      for (unsigned index = 0; index < I->getNumOperands(); ++index) {
        ...
        // constant expression.
        //
        if (ConstantExpr *CE = hasConstantIntToPtr(I->getOperand(index))) {
