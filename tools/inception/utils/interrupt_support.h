...
#include "inception/Transforms/NameRecovery.h"
...
class InterruptSupport {
 public:
  static void WriteInterruptPrologue(llvm::StringRef handler) {
    ...
    if (function == NULL) {
      klee::klee_warning("Cannot write IRQ prologue for unknonwn function %s.",
                        handler.str().c_str());
      return;
    }

    if (function->empty()) {
      klee::klee_message("Writing interrupt prologue to %s failed",
                        handler.str().c_str());
      return;
    }

    klee::klee_message("Writing interrupt prologue to %s",
                      handler.str().c_str());
  ...
  static void WriteInterruptEpilogue(llvm::StringRef handler) {
    ...
    if (function == NULL) {
      klee::klee_warning("Cannot write IRQ epilogue for unknonwn function %s.",
                        handler.str().c_str());
      return;
    }

    if (function->empty()) {
      klee::klee_message("Writing interrupt epilogue to %s failed",
                        handler.str().c_str());
      return;
    }

    klee::klee_message("Writing interrupt epilogue to %s",
                      handler.str().c_str());

    Instruction* last = GetLastInstruction(function);
    if (last == NULL) {
      klee::klee_error("Handler %s has no return instruction",
                      handler.str().c_str());
    }
