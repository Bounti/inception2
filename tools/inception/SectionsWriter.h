...
#include "inception/CodeInv/FractureMemoryObject.h"
...
class SectionsWriter {
 public:
  static void WriteSection(StringRef SectionName, const Disassembler* Dis,
    ...
    if (Section == NULL) {
      klee::klee_warning("[SectionsWriter] Section '%s' not found",
                        SectionName.str().c_str());
      return;
    }

    if (Section->getSize() == 0) {
      klee::klee_warning("[SectionsWriter] Section.getSize()");
      return;
    }

    std::error_code ec = Section->getContents(Bytes);
    if (ec) {
      klee::klee_error("[SectionsWriter] %s", ec.message().c_str());
    }
    ...
    klee::klee_message("%s ...", SectionName.str().c_str());
    ...
    if (SectionName.equals(".heap")) {
      klee::klee_message("%s declared but not initialised",
                        SectionName.str().c_str());
      return;
    }
